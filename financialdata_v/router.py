...
sys.path.append('./')
print(sys.path)
from financialdata_v import clients
...
class Router:
    def __init__(self):
        ...
        self._mysql_mlb_data_conn = (
            clients.get_mysql_mlb_data_conn()
        )
    ...
    def check_mysql_mlb_data_conn_alive(
        self,
    ):
        self._mysql_mlb_data_conn = check_connect_alive(
            self._mysql_mlb_data_conn,
            clients.get_mysql_mlb_data_conn,
        )
        return (
    ...
    @property
    def mysql_mlb_data_conn(self):
        """
        使用 property，在每次拿取 connect 時，
        都先經過 check alive 檢查 connect 是否活著
        """
        return (
            self.check_mysql_mlb_data_conn_alive()
        )